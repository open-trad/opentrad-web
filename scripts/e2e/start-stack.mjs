...
process.title = `opentrad-e2e-${runId}`;
...
async function close() {
  ...
  if (database && !databaseClosed) {
    try {
      ...
      // Continue removing run-owned artifacts after a close failure.
    }
    databaseClosed = true;
  }
...
try {
  ...
  app = await buildServer(config, { auth, jobs, logStream });
