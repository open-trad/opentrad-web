...
export type { DocxPlanBlockV2, DocxPlanSectionV2, DocxPlanV2 } from "./buildDocxPlan";
export { buildDocxPlanV2 } from "./buildDocxPlan";
...
export async function renderDocxV2(
  ...
  try {
    ...
    return await renderDocxPlanV2(plan, docx);
  } catch {
    throw new DocxV2GenerationError();
  }
}

async function renderDocxPlanV2(plan: DocxPlanV2, docx: typeof import("docx")): Promise<Blob> {
  ...
  function renderBlock(block: DocxPlanBlockV2, availableWidth: number): import("docx").FileChild[] {
    switch (block.type) {
      case "cover":
        return [
  ...
  const sections = plan.sections.map((section, sectionIndex): import("docx").ISectionOptions => {
    ...
    for (const block of section.blocks) children.push(...renderBlock(block, availableWidth));
