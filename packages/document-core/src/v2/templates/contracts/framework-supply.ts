...
import { calculateQuoteLinesV2, CurrencyV2Schema, formatMoneyMinorV2, TaxModeV2Schema } from "../../money.js";
...
import { DateV2Schema, type GoodsLineV2, GoodsLinesV2Schema } from "../quote-common.js";
import {
...
const TermSchema = strictContractObject(
  ...
  (term, addIssue) => {
    if (term.endDate < term.startDate) {
      addIssue({ code: "custom", message: "Framework term end must not precede start", path: ["endDate"] });
    }
  },
);
...
const FrameworkSupplyDraftRawSchema = strictContractObject(
  ...
  (draft, addIssue) => {
    if (draft.meta.language !== "zh-CN" || draft.meta.layoutStyleId !== "classic-formal.v1") {
      addIssue({ code: "custom", message: "Framework contract presentation is fixed", path: ["meta"] });
    }
...
export const FRAMEWORK_SUPPLY_CONTRACT_DEFINITION = {
  ...
  fieldManifest: [
    { path: "catalogLines", section: "catalog-price", label: "供应目录", control: "repeatable", required: true },
    { path: "forecast.binding", section: "forecast", label: "预测是否具有约束力", control: "checkbox", required: true },
    { path: "ordering.documentPriority", section: "orders-priority", label: "文件优先级", control: "textarea", required: true },
    { path: "performance.supplyContinuity", section: "continuity", label: "连续供应", control: "textarea", required: true },
    { path: "orderTemplateAttachmentId", section: "order-template", label: "订单模板", control: "attachment", required: false },
  ],
...
function createFrameworkDraft(input: { readonly id: string; readonly now: string | Date }) {
  ...
  return parseFrameworkDraft({
    ...
    meta: { contractNumber: "待填写", title: "框架供应合同", signingDate: dates.signingDate, effectiveMode: "signature", copies: 2, language: "zh-CN", layoutStyleId: "classic-formal.v1" },
    ...
    catalogLines: [{ id: "catalog-1", name: "待填写", unit: "待填写", quantity: "1", unitPriceMinor: "0", discountBps: 0, taxRateBps: 0 }],
    ...
    performance: { delivery: "", acceptance: "", reconciliationCycle: "", invoice: "", settlement: "", quality: "", warranty: "", supplyContinuity: "" },
    generalTerms: {
      noticeAddresses: "待填写", confidentiality: "待填写", forceMajeure: "待填写", changeControl: "待填写", termination: "待填写", breachRemedies: "待填写", governingLaw: "待填写", disputeMethod: "court", court: "待填写", severability: "待填写", entireAgreement: "待填写",
    },
    signers: [
      { partyId: "supplier", role: localized("供应商"), dateLabel: localized("日期"), sealLabel: localized("盖章") },
      { partyId: "purchaser", role: localized("采购方"), dateLabel: localized("日期"), sealLabel: localized("盖章") },
    ],
    attachments: [],
...
function analyzeFrameworkDraft(draft: FrameworkSupplyContractDraftV1): readonly RiskFindingV2[] {
  ...
  block(!draft.pricing.currency, "CONTRACT_CURRENCY_MISSING", "必须由用户选择合同币种", ["pricing", "currency"]);
  block(!draft.pricing.taxMode, "CONTRACT_TAX_MODE_MISSING", "必须由用户选择含税口径", ["pricing", "taxMode"]);
  block(!draft.forecast.frequency.trim(), "FRAMEWORK_FORECAST_FREQUENCY_MISSING", "必须填写预测频率", ["forecast", "frequency"]);
  block(!draft.ordering.documentPriority.trim(), "FRAMEWORK_ORDER_PRIORITY_MISSING", "必须填写订单与框架文件优先级", ["ordering", "documentPriority"]);
  block(!draft.performance.supplyContinuity.trim(), "FRAMEWORK_CONTINUITY_MISSING", "必须填写连续供应安排", ["performance", "supplyContinuity"]);
  if (
    ...
    findings.push(contractFinding("FRAMEWORK_COMMERCIAL_RISK_UNCONFIRMED", "warning", "watermark", "最低采购或独家安排须经用户单独确认", ["riskAcknowledgements", "commercialRiskConfirmed"]));
  }
...
function compileFrameworkDraft(value: unknown): DocumentModelV2 {
  ...
  const calculation = draft.pricing.currency && draft.pricing.taxMode
    ? calculateQuoteLinesV2(draft.catalogLines.map((line) => ({ id: line.id, quantity: line.quantity, unitPriceMinor: line.unitPriceMinor, discountBps: line.discountBps, taxRateBps: line.taxRateBps })), { currency: draft.pricing.currency, taxMode: draft.pricing.taxMode })
    : undefined;
  const calculated = new Map(calculation?.lines.map((line) => [line.lineId, line.totalMinor]));
  const money = (minor: string) => draft.pricing.currency ? formatMoneyMinorV2(minor, draft.pricing.currency) : "待选择币种";
  const paragraph = (id: string, value: string) => ({ type: "paragraph" as const, id, text: localized(value) });
  const section = (id: string, value: string) => ({ id, blocks: [paragraph(`${id}-text`, value)] });
  const sections = [
    { id: "cover", blocks: [{ type: "cover" as const, id: "framework-cover", title: localized(draft.meta.title), subtitle: localized(draft.meta.contractNumber) }] },
    { id: "meta", blocks: [{ type: "keyValueGrid" as const, id: "framework-meta", entries: [
    ...
    { id: "parties", blocks: [{ type: "parties" as const, id: "framework-parties", parties: [
      { id: "supplier", role: localized("供应商"), name: localized(draft.supplier.legalName), details: partyDetails(draft.supplier) },
      { id: "purchaser", role: localized("采购方"), name: localized(draft.purchaser.legalName), details: partyDetails(draft.purchaser) },
    ] }] },
    section("framework-purpose", "本合同约定持续供应的一般规则；具体交易以依约成立的采购订单为准。"),
    section("term", `${draft.term.startDate}至${draft.term.endDate}`),
    { id: "catalog-price", blocks: [
      { type: "table" as const, id: "catalog-table", columns: [
        { id: "name", label: localized("目录商品"), width: "35%", align: "left" as const },
        { id: "quantity", label: localized("参考数量"), width: "20%", align: "right" as const },
        { id: "unitPrice", label: localized("目录单价"), width: "20%", align: "right" as const },
        { id: "amount", label: localized("参考金额"), width: "25%", align: "right" as const },
      ], rows: draft.catalogLines.map((line) => ({ id: line.id, cells: { name: localized(line.name), quantity: localized(`${line.quantity} ${line.unit}`), unitPrice: localized(money(line.unitPriceMinor)), amount: localized(calculated.has(line.id) ? money(calculated.get(line.id) ?? "0") : "待完善计价选择") } })), repeatHeader: true, pagePolicy: { allowRowSplit: false, keepHeaderWithRows: 1 } },
      { type: "paragraph" as const, id: "pricing-method", text: localized(`价格机制：${text(draft.pricing.priceMethod)}；调整触发：${text(draft.pricing.adjustmentTrigger)}；提前${draft.pricing.adjustmentNoticeDays}日通知。`) },
      { type: "totals" as const, id: "catalog-total", entries: [{ id: "total", label: localized("目录参考合计"), value: localized(calculation ? money(calculation.summary.totalMinor) : "待完善计价选择") }] },
    ] },
    section("forecast", `${draft.forecast.frequency || "待填写"}。${draft.forecast.binding ? "双方明确约定该预测具有约束力。" : "预测和目录不当然构成采购义务。"}`),
    section("minimum-or-exclusivity", `最低采购：${text(draft.forecast.minimumPurchaseCommitment)}；独家：${text(draft.forecast.exclusivity)}`),
    section("orders-priority", `订单成立：${text(draft.ordering.formation)}；审批：${text(draft.ordering.approval)}；文件优先级：${text(draft.ordering.documentPriority)}`),
    section("capacity-inventory", `MOQ：${text(draft.ordering.moq)}；交期：${text(draft.ordering.leadTime)}；产能：${text(draft.ordering.capacityCommitment)}；库存：${text(draft.ordering.inventoryPolicy)}`),
    section("delivery-acceptance", `交付：${text(draft.performance.delivery)}；验收：${text(draft.performance.acceptance)}`),
    section("reconciliation-payment", `对账：${text(draft.performance.reconciliationCycle)}；发票：${text(draft.performance.invoice)}；结算：${text(draft.performance.settlement)}`),
    section("quality-warranty", `质量：${text(draft.performance.quality)}；质保：${text(draft.performance.warranty)}`),
    section("continuity", text(draft.performance.supplyContinuity)),
    section("change-termination-transition", `变更：${draft.generalTerms.changeControl}；终止：${draft.generalTerms.termination}；过渡：${text(draft.performance.transitionAssistance)}`),
    { id: "general-terms", blocks: [{ type: "list" as const, id: "framework-general-list", ordered: false, items: [localized(draft.generalTerms.confidentiality), localized(draft.generalTerms.forceMajeure), localized(draft.generalTerms.breachRemedies), localized(draft.generalTerms.governingLaw), localized(draft.generalTerms.noticeAddresses)] }] },
    { id: "order-template", blocks: [{ type: "attachmentIndex" as const, id: "order-template-index", attachmentIds: draft.orderTemplateAttachmentId ? [draft.orderTemplateAttachmentId] : [] }] },
    { id: "signatures", blocks: [{ type: "signatureGroup" as const, id: "framework-signatures", signers: signerBlocks(draft.signers, { supplier: draft.supplier, purchaser: draft.purchaser }) }] },
  ];
  return DocumentModelV2Schema.parse({
    ...
    sections, watermarks: contractWatermarks(findings), disclaimers: ["contract-generation-note"],
    attachmentManifest: exportedAttachments(draft.attachments),
  }) as DocumentModelV2;
}

export const FRAMEWORK_SUPPLY_CONTRACT_REGISTRATION: TemplateRegistration<unknown, DocumentModelV2> = Object.freeze({
  ...
  preflight(value: unknown) { return analyzeFrameworkDraft(parseFrameworkDraft(value)); },
});
