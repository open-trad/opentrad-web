...
const DomesticSaleContractDraftRawSchema = strictContractObject(
  ...
  (draft, addIssue) => {
    if (draft.meta.language !== "zh-CN" || draft.meta.layoutStyleId !== "classic-formal.v1") {
      addIssue({ code: "custom", message: "Domestic contract presentation is fixed", path: ["meta"] });
    }
...
export const DOMESTIC_SALE_CONTRACT_DEFINITION = {
  ...
  fieldManifest: [
    { path: "goodsLines", section: "subject-goods", label: "商品明细", control: "repeatable", required: true },
    { path: "price.currency", section: "price-tax-invoice", label: "币种", control: "select", required: true },
    { path: "price.taxMode", section: "price-tax-invoice", label: "计税口径", control: "select", required: true },
    { path: "price.invoiceType", section: "price-tax-invoice", label: "发票类型", control: "select", required: true },
    { path: "delivery.riskTransfer", section: "title-risk", label: "风险转移", control: "textarea", required: true },
    { path: "acceptance.inspectionPeriod", section: "inspection-acceptance", label: "检验期限", control: "textarea", required: true },
  ],
...
function createDomesticDraft(input: { readonly id: string; readonly now: string | Date }) {
  ...
  return parseDomesticDraft({
    ...
    signers: [
      { partyId: "seller", role: localized("卖方"), dateLabel: localized("签署日期"), sealLabel: localized("盖章") },
      { partyId: "buyer", role: localized("买方"), dateLabel: localized("签署日期"), sealLabel: localized("盖章") },
    ],
    attachments: [],
...
function analyzeDomesticDraft(draft: DomesticSaleContractDraftV1): readonly RiskFindingV2[] {
  ...
  required(!draft.price.currency, "CONTRACT_CURRENCY_MISSING", "必须由用户选择合同币种", ["price", "currency"]);
  required(!draft.price.taxMode, "CONTRACT_TAX_MODE_MISSING", "必须由用户选择含税口径", ["price", "taxMode"]);
  required(!draft.price.invoiceType, "DOMESTIC_INVOICE_TYPE_MISSING", "必须由用户选择发票类型", ["price", "invoiceType"]);
  required(!draft.price.invoiceTiming.trim(), "DOMESTIC_INVOICE_TIMING_MISSING", "必须填写开票时点", ["price", "invoiceTiming"]);
  required(!draft.delivery.riskTransfer.trim(), "DOMESTIC_RISK_TRANSFER_MISSING", "必须填写风险转移规则", ["delivery", "riskTransfer"]);
  required(!draft.acceptance.inspectionPeriod.trim(), "DOMESTIC_INSPECTION_PERIOD_MISSING", "必须填写检验期限", ["acceptance", "inspectionPeriod"]);
  required(!draft.acceptance.objectionMethod.trim(), "DOMESTIC_OBJECTION_METHOD_MISSING", "必须填写异议方式", ["acceptance", "objectionMethod"]);
  required(draft.goodsLines.some((line) => line.taxRateBps === undefined), "DOMESTIC_TAX_RATE_MISSING", "税率须由用户逐项填写", ["goodsLines"]);
...
function compileDomesticDraft(value: unknown): DocumentModelV2 {
  ...
  const sections = [
    { id: "cover", blocks: [{ type: "cover" as const, id: "contract-cover", title: localized(draft.meta.title), subtitle: localized(draft.meta.contractNumber) }] },
    { id: "meta", blocks: [{ type: "keyValueGrid" as const, id: "contract-meta", entries: [
      { id: "number", label: localized("合同编号"), value: localized(draft.meta.contractNumber) },
      { id: "date", label: localized("签署日期"), value: localized(draft.meta.signingDate) },
      { id: "place", label: localized("签署地点"), value: localized(display(draft.meta.signingPlace)) },
      { id: "copies", label: localized("合同份数"), value: localized(String(draft.meta.copies)) },
    ] }] },
    { id: "parties", blocks: [{ type: "parties" as const, id: "contract-parties", parties: [
      { id: "seller", role: localized("卖方"), name: localized(draft.seller.legalName), details: partyDetails(draft.seller) },
      { id: "buyer", role: localized("买方"), name: localized(draft.buyer.legalName), details: partyDetails(draft.buyer) },
    ] }] },
    { id: "subject-goods", blocks: [{ type: "table" as const, id: "goods-table", columns: [
      { id: "name", label: localized("商品"), width: "24%", align: "left" as const },
      { id: "spec", label: localized("品牌/规格/标准"), width: "30%", align: "left" as const },
    ...
    ], rows: draft.goodsLines.map((line) => ({ id: line.id, cells: {
      name: localized(line.name),
      spec: localized([line.brand, line.specification, line.qualityStandard].filter(Boolean).join(" / ")),
      quantity: localized(`${line.quantity} ${line.unit}`),
      unitPrice: localized(money(line.unitPriceMinor)),
      amount: localized(calculatedById.has(line.id) ? money(calculatedById.get(line.id)?.totalMinor ?? "0") : "待完善计价选择"),
    } })), repeatHeader: true, pagePolicy: { allowRowSplit: false, keepHeaderWithRows: 1 } }] },
    { id: "price-tax-invoice", blocks: [
      { type: "keyValueGrid" as const, id: "price-grid", entries: [
        { id: "currency", label: localized("币种"), value: localized(draft.price.currency ?? "待选择") },
        { id: "tax", label: localized("含税口径"), value: localized(draft.price.taxMode ?? "待选择") },
        { id: "invoice", label: localized("发票类型"), value: localized(draft.price.invoiceType ?? "待选择") },
        { id: "timing", label: localized("开票时点"), value: localized(display(draft.price.invoiceTiming)) },
      ] },
      { type: "totals" as const, id: "contract-total", entries: [{ id: "total", label: localized("合同总价"), value: localized(calculation ? money(calculation.summary.totalMinor) : "待完善计价选择") }] },
    ] },
    { id: "payment", blocks: [
      { type: "list" as const, id: "payment-schedule", ordered: true, items: draft.price.paymentSchedule.map((item) => localized(`${item.trigger}：${(item.amountBps / 100).toFixed(2)}%，${item.dueDays}日内`)) },
      ...(draft.price.retentionBps === undefined ? [] : [{ type: "notice" as const, id: "retention-note", tone: "info" as const, paragraphs: [localized(`留存比例为 ${(draft.price.retentionBps / 100).toFixed(2)}%；留存比例属于付款进度分配，不从合同总价重复扣减。`)] }]),
    ] },
    { id: "delivery-packaging", blocks: [{ type: "clauseGroup" as const, id: "delivery-clauses", title: localized("交付与包装"), clauses: [
      { id: "delivery", number: "1", title: localized("交付"), paragraphs: [localized(`${display(draft.delivery.time)}；${display(draft.delivery.place)}；${draft.delivery.method}`)] },
      { id: "packaging", number: "2", title: localized("包装与费用"), paragraphs: [localized(`${display(draft.delivery.packaging)}；运费：${display(draft.delivery.freightAllocation)}；保险：${display(draft.delivery.insuranceAllocation)}`)] },
    ] }] },
    { id: "title-risk", blocks: [
      { type: "paragraph" as const, id: "title-transfer", text: localized(`所有权转移：${display(draft.delivery.titleTransfer)}`) },
      { type: "paragraph" as const, id: "risk-transfer", text: localized(`风险转移：${display(draft.delivery.riskTransfer)}`) },
    ] },
    { id: "inspection-acceptance", blocks: [{ type: "clauseGroup" as const, id: "inspection-clauses", title: localized("检验与异议"), clauses: [
      { id: "inspection", number: "1", title: localized("检验"), paragraphs: [localized(`${display(draft.acceptance.inspectionStandard)}；${display(draft.acceptance.inspectionMethod)}；期限：${display(draft.acceptance.inspectionPeriod)}`)] },
      { id: "objection", number: "2", title: localized("异议"), paragraphs: [localized(display(draft.acceptance.objectionMethod))] },
    ] }] },
    { id: "quality-warranty", blocks: [{ type: "paragraph" as const, id: "quality-warranty-text", text: localized(`质保：${display(draft.acceptance.warranty)}；售后：${display(draft.acceptance.afterSales)}`) }] },
    { id: "parties-obligations", blocks: [{ type: "paragraph" as const, id: "obligations-text", text: localized("双方应按约提供履约所需资料、协助并及时通知影响履约的事项。") }] },
    { id: "breach-termination", blocks: [{ type: "clauseGroup" as const, id: "breach-termination-clauses", title: localized("违约与解除"), clauses: [
      { id: "breach", number: "1", title: localized("违约责任"), paragraphs: [localized(draft.generalTerms.breachRemedies)] },
      { id: "termination", number: "2", title: localized("解除"), paragraphs: [localized(draft.generalTerms.termination)] },
    ] }] },
    { id: "force-majeure", blocks: [{ type: "paragraph" as const, id: "force-majeure-text", text: localized(draft.generalTerms.forceMajeure) }] },
    { id: "notices", blocks: [{ type: "paragraph" as const, id: "notices-text", text: localized(draft.generalTerms.noticeAddresses) }] },
    { id: "governing-law-dispute", blocks: [{ type: "paragraph" as const, id: "law-dispute-text", text: localized(`${draft.generalTerms.governingLaw}；${draft.generalTerms.disputeMethod === "court" ? draft.generalTerms.court : draft.generalTerms.arbitrationCommission}`) }] },
    { id: "miscellaneous", blocks: [{ type: "list" as const, id: "misc-list", ordered: false, items: [localized(draft.generalTerms.confidentiality), localized(draft.generalTerms.changeControl), localized(draft.generalTerms.severability), localized(draft.generalTerms.entireAgreement)] }] },
    { id: "attachments", blocks: [{ type: "attachmentIndex" as const, id: "attachment-index", attachmentIds: draft.attachments.map((attachment) => attachment.id) }] },
    { id: "signatures", blocks: [{ type: "signatureGroup" as const, id: "contract-signatures", signers: signerBlocks(draft.signers, { seller: draft.seller, buyer: draft.buyer }) }] },
  ];
...
export const DOMESTIC_SALE_CONTRACT_REGISTRATION: TemplateRegistration<unknown, DocumentModelV2> = Object.freeze({
