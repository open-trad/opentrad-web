...
mkdir -p "$temporary/live/opentrad.dynv6.net"
openssl req -x509 -newkey rsa:2048 -nodes -days 1 \
  -subj '/CN=opentrad.dynv6.net' \
  -keyout "$temporary/live/opentrad.dynv6.net/privkey.pem" \
  -out "$temporary/live/opentrad.dynv6.net/fullchain.pem" >/dev/null 2>&1
