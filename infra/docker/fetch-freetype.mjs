...
if (!lockPath || !targetPath) {
  process.stderr.write("usage: fetch-freetype.mjs LOCK TARGET\n");
  process.exit(64);
}
...
if (
  ...
  lock.license !== "FTL OR GPL-2.0-only" ||
  !/^[a-f0-9]{64}$/.test(lock.sha256) ||
  Object.keys(lock).sort().join(",") !== "id,license,schemaVersion,sha256,source,version"
) {
  throw new Error("invalid FreeType lock");
}

const archive = resolve(targetPath, `freetype-${process.pid}.tar.xz`);
...
try {
  ...
  if (!response.ok || !response.body) throw new Error("FreeType download failed");
  ...
  if (hash.digest("hex") !== lock.sha256) throw new Error("FreeType checksum mismatch");
  renameSync(temporary, archive);
  const extraction = spawnSync("tar", ["-xJf", archive, "-C", targetPath, "--strip-components=1"], {
    stdio: "inherit",
  });
  if (extraction.status !== 0) throw new Error("FreeType extraction failed");
} finally {
