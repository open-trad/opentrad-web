...
if (!lockPath || !targetPath) {
  process.stderr.write("usage: fetch-expat.mjs LOCK TARGET\n");
  process.exit(64);
}
...
if (
  ...
  throw new Error("invalid Expat lock");
}

const archive = resolve(targetPath, `expat-${process.pid}.tar.xz`);
...
try {
  ...
  if (!response.ok || !response.body) throw new Error("Expat download failed");
  ...
  if (hash.digest("hex") !== lock.sha256) throw new Error("Expat checksum mismatch");
  renameSync(temporary, archive);
  const extraction = spawnSync("tar", ["-xJf", archive, "-C", targetPath, "--strip-components=1"], {
    stdio: "inherit",
  });
  if (extraction.status !== 0) throw new Error("Expat extraction failed");
} finally {
